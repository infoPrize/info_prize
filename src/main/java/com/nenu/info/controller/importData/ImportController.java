...
public class ImportController extends AbstractController{
    ...
    public String importStudent(@RequestParam("file")MultipartFile file , RedirectAttributes redirectAttributes , HttpServletRequest request , Model model){
        try{
            ...
            File excel = FileUtil.create(request.getSession().getServletContext().getRealPath("resources/upload/student/")+file.getOriginalFilename());//本地调试，必须制定特定的存在目录，否则找不到存储文件的位置
            int importNum = 0;
    ...
    public String importThesis(@RequestParam("file")MultipartFile file , Model model, HttpServletRequest request ){
        try{
            File excel = FileUtil.create(request.getSession().getServletContext().getRealPath("resources/upload/thesis/")+file.getOriginalFilename());//本地调试，必须制定特定的存在目录，否则找不到存储文件的位置
            int importNum = 0;
            file.transferTo(excel);
    ...
    public String importPatent(@RequestParam("file")MultipartFile file ,  HttpServletRequest request , Model model){
        int importNum = 0;
        try{
//           File excel = FileUtil.create(file.getOriginalFilename());//服务器端
            File excel = FileUtil.create(request.getSession().getServletContext().getRealPath("resources/upload/patent/")+file.getOriginalFilename());//本地调试，必须制定特定的存在目录，否则找不到存储文件的位置
            file.transferTo(excel);
    ...
    public String importScientificProject(@RequestParam("file")MultipartFile file ,  HttpServletRequest request , Model model){
        int importNum = 0;
        try{
            File excel = FileUtil.create(request.getSession().getServletContext().getRealPath("resources/upload/scientificProject/")+file.getOriginalFilename());//本地调试，必须制定特定的存在目录，否则找不到存储文件的位置
    ...
    public String importMath(@RequestParam("file")MultipartFile file , Model model , HttpServletRequest request ){
        int importNum = 0;
        try{
            File excel = FileUtil.create(request.getSession().getServletContext().getRealPath("resources/upload/math/")+file.getOriginalFilename());//本地调试，必须制定特定的存在目录，否则找不到存储文件的位置
            file.transferTo(excel);
    ...
    public String importInternetPlus(@RequestParam("file")MultipartFile file , Model model , HttpServletRequest request ){
        int importNum = 0;
        try{
            File excel = FileUtil.create(request.getSession().getServletContext().getRealPath("resources/upload/internetPlus/")+file.getOriginalFilename());//本地调试，必须制定特定的存在目录，否则找不到存储文件的位置
            file.transferTo(excel);
    ...
    public String importChallengeCup(@RequestParam("file")MultipartFile file , Model model , HttpServletRequest request ){
        int importNum = 0;
        try{
            File excel = FileUtil.create(request.getSession().getServletContext().getRealPath("resources/upload/challengeCup/")+file.getOriginalFilename());//本地调试，必须制定特定的存在目录，否则找不到存储文件的位置
            file.transferTo(excel);
    ...
    public String importOtherMatch(@RequestParam("file")MultipartFile file , Model model , HttpServletRequest request ){
        int importNum = 0;
        try{
            File excel = FileUtil.create(request.getSession().getServletContext().getRealPath("resources/upload/otherMatch/")+file.getOriginalFilename());//本地调试，必须制定特定的存在目录，否则找不到存储文件的位置
            file.transferTo(excel);
